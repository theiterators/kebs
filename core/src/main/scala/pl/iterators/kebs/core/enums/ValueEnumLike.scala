...
trait ValueEnumLike[ValueType, EntryType <: ValueEnumLikeEntry[ValueType]] {
  ...
  def withValue(i: ValueType): EntryType = withValueOption(i).getOrElse(throw new NoSuchElementException(ValueEnumLike.buildNotFoundMessage(this, i)))
...
private[core] object ValueEnumLike {
  private def valuesToEntriesMap[ValueType, EntryType <: ValueEnumLikeEntry[ValueType]](`enum`: ValueEnumLike[ValueType, EntryType]): Map[ValueType, EntryType] = `enum`.values.map(v => v.value -> v).toMap
  private def existingEntriesString[ValueType, EntryType <: ValueEnumLikeEntry[ValueType]](`enum`: ValueEnumLike[ValueType, EntryType]): String = `enum`.values.map(_.value).mkString(", ")
  private def buildNotFoundMessage[ValueType, EntryType <: ValueEnumLikeEntry[ValueType]](`enum`: ValueEnumLike[ValueType, EntryType], i: ValueType): String = s"${i.toString} is not a member of ValueEnum (${existingEntriesString(`enum`)})"
}
