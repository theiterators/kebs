...
trait EnumLike[T] {
  ...
  def withNameUppercaseOnly(name: String): T = withNameUppercaseOnlyOption(name).getOrElse(throw new NoSuchElementException(EnumLike.buildNotFoundMessage(name, this)))
  def withNameLowercaseOnly(name: String): T = withNameLowercaseOnlyOption(name).getOrElse(throw new NoSuchElementException(EnumLike.buildNotFoundMessage(name, this)))
  def withName(name: String): T = withNameOption(name).getOrElse(throw new NoSuchElementException(EnumLike.buildNotFoundMessage(name, this)))
...
private[core] object EnumLike {
  ...
  private def buildNotFoundMessage[T](notFoundName: String, `enum`: EnumLike[T]): String = s"$notFoundName is not a member of Enum (${existingEntriesString(`enum`)})"
}
