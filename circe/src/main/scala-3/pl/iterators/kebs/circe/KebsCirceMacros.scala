...
import scala.quoted.*
 ...
 abstract class KebsCirceMacros {
  ...
  def caseAccessors[T: Type](using Quotes): List[quotes.reflect.Symbol] = {
    ...
    tpe.typeSymbol.declaredMethods
  }

  inline final def materializeDecoder[T](using quotes: Quotes, m: Mirror.Of[T]): Decoder[T] = {
    import quotes.reflect.*
    ...
    if !tpeSym.flags.is(Flags.Case) then scala.sys.error(s"Not a case class: ${tpeSym.flags.show}")
x.
    val decoder = caseAccessors[T] match {
      case Nil =>
        ${_root_.io.circe.Decoder.decodeJsonObject.emap(obj => if(obj.isEmpty) Right(Expr(x.)) else Left("Empty JsonObject"))}
      case _1 :: Nil =>
